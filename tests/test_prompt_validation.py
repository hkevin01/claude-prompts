...
"""
    ...
    def test_parse_frontmatter_valid(self, validator, valid_prompt_content):
        ...
        frontmatter, content = validator.parse_frontmatter(
...
Content here"""
    ...
    def test_category_validation(self, validator):
        ...
        for category in valid_categories:
            ...
            validator.validate_frontmatter_schema(frontmatter)
            category_errors = [e for e in validator.errors if 'category' in e]
            msg = f"Category '{category}' should be valid"
            assert len(category_errors) == 0, msg    def test_difficulty_validation(self, validator):
        ...
        for difficulty in valid_difficulties:
            ...
            validator.validate_frontmatter_schema(frontmatter)
            difficulty_errors = [e for e in validator.errors if 'difficulty' in e]
            assert len(difficulty_errors) == 0, f"Difficulty '{difficulty}' should be valid"
    ...
    def test_tags_validation(self, validator):
        ...
        for tags, should_be_valid in test_cases:
            ...
            if should_be_valid:
                assert len(tags_errors) == 0, f"Tags {tags} should be valid"
            else:
                assert len(tags_errors) > 0, f"Tags {tags} should be invalid"

    def test_validate_file_with_temp_file(self, validator, valid_prompt_content):
    ...
    def test_date_validation(self, validator, date_string, expected_valid):
        ...
        if expected_valid:
            assert len(date_errors) == 0, f"Date '{date_string}' should be valid"
        else:
            assert len(date_errors) > 0, f"Date '{date_string}' should be invalid"
...
class TestIntegration:
    ...
    def test_validate_directory_structure(self):
        ...
        if src_prompts.exists():
            categories = [d.name for d in src_prompts.iterdir() if d.is_dir()]
            expected_categories = ['coding', 'creative', 'business',
                                 'analysis', 'educational', 'personal']

            for category in expected_categories:
                assert category in categories, f"Category '{category}' directory should exist"
